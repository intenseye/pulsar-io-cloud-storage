...
| `partitionerType`               | String  | False    | "partition"  | The partitioning type. It can be configured by topic partitions, by time or by values of fields configured. By default, the partition type is configured by topic partitions.                                                                                                                                                                                                                                         |
...
| `fieldsPartitionList`           | List&lt;String&gt; | True | ["key1", "key2"] | The keys inside the record to create path hierarchy from, example results in value1/value2/ as a path prefix from their respective values. Required and only effective when `partitionerType` is `"fields"`.                                                                                                                                                                                                                                                                                       |
| `fieldsPartitionIgnoreMissing`  | Boolean | False    | true         | Whether to ignore the missing fields and disregard them or throw errors for missing values                                                                                                                                                                                                                                                                                            |
